...
test.describe('Team flow', () => {
    test('should allow completing the full CRUD flow for a team using mocks', async ({ page }) => {
        ...
        await page.route('**/api/matches/team/**', async (route) => {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify([]),
        ...
        await page.route('**/api/teams', async (route) => {
            ...
            if (method === 'GET') {
                ...
                const newTeam = {
                    ...
                    stadiumId: payload.stadiumId ? Number(payload.stadiumId) : null,
                    stadium: payload.stadiumId ? stadiums.find((s) => s.id === Number(payload.stadiumId)) || null : null,
                    players: [],
                    homeMatches: [],
                    awayMatches: [],
                };
            ...
            if (method === 'GET') {
                ...
                if (index !== -1) {
                    teams[index] = {
                        ...
                        stadiumId: payload.stadiumId !== undefined ? Number(payload.stadiumId) : teams[index].stadiumId,
                        stadium: payload.stadiumId ? stadiums.find((s) => s.id === Number(payload.stadiumId)) || null : teams[index].stadium,
                    };
                    await route.fulfill({
                        status: 200,
                        contentType: 'application/json',
        ...
        await page.getByLabel('Estadio Habitual').selectOption({ label: 'Spotify Camp Nou (Capacidad: 99,354)' });
        ...
        await expect(page.getByText('Estadio Habitual: Spotify Camp Nou')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Plantilla de Jugadores' })).toBeVisible();
        await expect(page.getByText('No hay jugadores registrados en este equipo.')).toBeVisible();
